...
const PendingPackages = (props) => {
  ...
  const send = async (p) => {
    if (await packageRequests.serve(p)) {
      snackbar.showSuccess("Package successfully added.")
    } else {
      snackbar.showError(
        "Unable to approve this package! May be the balance is insufficient!"
      )
    }
    setSending(false)
  }

  const _clear = async (p) => {
    if (await packageRequests.clear(p)) {
      snackbar.showSuccess("Message is cleared successfully.")
    } else {
      snackbar.showError("Unable to clear the message")
    }
    setSending(false)
  }
