...
const Dash = props => {
    ...
    React.useEffect(() => {
        if (!users.list.length) {
            snackbar.showInfo('No pending approval.')
            setBusy(true)
            users.load(login.info).finally(() => setBusy(false))
        } else {
                    ...
                    <div className='grid md:grid-cols-3 sm:grid-cols-1 gap-2'>
                        {
                            !users.list.length
                                ?
                                <div className='text-slate-400 text-2xl font-extrabold'>No new registration</div>
                                :
                                users.list.map(u => (
                                    <UserCard
                                        user={u}
                                        ...
                                        onSend={amount => {
                                            ...
                                            confirm.open(
                                                <div className='p-2'>
                                                    <div className='text-orange-600 text-center animate-pulse duration-75'><span className='font-extrabold'>Warning!</span> Once applied the process cannot be revert back.</div>
