...
export default class Withdrawals {
  ...
  dispatchWithdrawals = () =>
    this.dispatch({
      type: "dispatchWithdrawals",
      payload: new Withdrawals(this),
    })

  constructor(withdrawals = null) {
    this.list =
      withdrawals && withdrawals.list
        ? withdrawals.list.map((u) => new Withdrawal(u))
        : []
    this.dispatch =
      withdrawals && withdrawals.dispatch ? withdrawals.dispatch : null
  }
   ...
   * @param {Withdrawal} withdrawal
   */
  clear = async (withdrawal) => {
    const w = await api("/admin/withdraw-clear", withdrawal.json())
    if (w && withdrawal.docid === w.docid) {
      this.list = this.list.filter(
        (w2) =>
          !(
            w2.docid === w.docid &&
