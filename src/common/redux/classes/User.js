...
export default class User {
    ...
    constructor(user = null) {
        ...
        //////////////////////////////////////////////////////////////
        this.groupA = user && user.groupA ? user.groupA.map(a => new Subordinate(a)) : []
        this.groupB = user && user.groupB ? user.groupB.map(b => new Subordinate(b)) : []
        this.packages = user && user.packages ? user.packages.map(p => new Package(p)) : []
        ...
        this.monthlyTurnover = user && user.monthlyTurnover ? user.monthlyTurnover : []
    }
    ...
    json() {
        const {
            ...
            totalWithdrawal, cryptoWalletAddress,
        } = this
        const groupA = this.groupA.map(a => a.json())
        const groupB = this.groupB.map(b => b.json())
        const packages = this.packages.map(p => p.json())
        const totalTurnover = (this.docid === ADMINDOCID) ? this.totalTurnover : undefined
        const monthlyTurnover = (this.docid === ADMINDOCID) ? this.monthlyTurnover : undefined
        const income = this.income.json()
        return ({
            appname, subscriberdocid, createdon, modifiedon,
            refdocid, docid, active, name, countrycode, mobile, email, password, idproof,
            groupA, groupB, packages, totalWithdrawal, cryptoWalletAddress,
